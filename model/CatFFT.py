...
class CATB_axial(nn.Module):
    def __init__(self, dim, reso, num_heads,
        ...
        self.qkv = nn.Linear(dim//4*3, dim//4 * 9, bias=qkv_bias)
        ...
        self.attns = nn.ModuleList([
            Attention_axial(
                dim // 8*3, resolution=self.patches_resolution, idx=i,
                split_size=split_size, num_heads=num_heads // 2, dim_out=dim // 8*3,
        ...
        self.get_v = dep_conv(in_channels=dim//4*3, kernel_size=3)  # Dep Conv
        ...
        self.f_attns = GlobalFrequencyBlock(in_channels=dim//4)
        self.c_attns = CAB(dim, compress_ratio=4, squeeze_factor=16, memory_blocks=256)
    ...
    def forward(self, x, x_size):  # x(1,9216,48) x_size[96,96]
        ...
        qkv = self.qkv(img[:,:,:C//4*3])  # qkv(1,9216,108)
        qkv = qkv.reshape(B, -1, 3, C//4*3).permute(2, 0, 1, 3)  # 3, B, HW, C qkv(3,1,9216,36)
        # v without partition
        v = qkv[2].transpose(-2, -1).contiguous().view(B, C//4*3, H, W)  # v(1,36,96,96)

        if self.shift_size > 0:
            qkv = qkv.view(3, B, H, W, C//4*3)
            # V-Shift
            qkv_0 = torch.roll(qkv[:, :, :, :, :C // 8*3], shifts=-self.shift_size, dims=3)
            qkv_0 = qkv_0.view(3, B, L, C // 8*3)
            # H-Shift
            qkv_1 = torch.roll(qkv[:, :, :, :, C // 8*3:C // 4*3], shifts=-self.shift_size, dims=2)
            qkv_1 = qkv_1.view(3, B, L, C // 8 * 3)
            ...
            x1 = x1.view(B, L, C // 8 * 3).contiguous()
            x2 = x2.view(B, L, C // 8 * 3).contiguous()
            # Concat
            attened_x = torch.cat([x1, x2], dim=2)
        else:
            # V-Rwin
            x1 = self.attns[0](qkv[:, :, :, :C // 8*3], H, W).view(B, L, C // 8 * 3).contiguous()  # x1(1,9216,18)
            # H-Rwin
            x2 = self.attns[1](qkv[:, :, :, C // 8*3: C // 4 * 3], H, W).view(B, L, C // 8 * 3).contiguous()  # x1(1,9216,18)
            # Concat
            attened_x = torch.cat([x1, x2], dim=2)  # x1(1,9216,24)

        # Locality Complementary Module
        lcm = self.get_v(v)  # lcm(1,)  # lcm(1,24,96,96)
        lcm = lcm.permute(0, 2, 3, 1).contiguous().view(B, L, C // 4 * 3)  # lcm(1,9216,48)
        attened_x = attened_x + lcm  # attened_x(1,9216,24)

        # 频率块
        attened_f = self.f_attns(img[:, :, C//4*3:], x_size)
        attened = torch.cat([attened_x, attened_f], dim=2)

        # 光谱分流模块
        attened = rearrange(attened, 'b n (g d) -> b n ( d g)', g=4)  # attened_x(1,9216,48)

        attened = self.proj(attened)
        attened = self.proj_drop(attened)  # attened_x(1,9216,48)

        attened_c = self.c_attns(img)
        x = x + self.drop_path(attened) + self.conv_scale * attened_c
...
class GlobalFrequencyBlock(nn.Module):
    def __init__(self, in_channels):
        # bn_layer not used
        super(GlobalFrequencyBlock, self).__init__()

        self.conv_1 = nn.Sequential(
            depthwise_conv(in_channels=in_channels, out_channels=in_channels//2),
            nn.GELU(),
            depthwise_conv(in_channels=in_channels//2, out_channels=in_channels)
        )
