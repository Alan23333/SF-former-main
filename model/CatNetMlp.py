# 更改了Mlp的网络，其中mlp_ratio=2的版本跑完了，暂时以其为准
import numpy as np
import torch
...
def img2windows(img, H_sp, W_sp):
    ...
    """
    B, C, H, W = img.shape
    img_reshape = img.view(B, C, H // H_sp, H_sp, W // W_sp, W_sp)  # (24,1,96,24,4)
    img_perm = img_reshape.permute(0, 2, 4, 3, 5, 1).contiguous().reshape(-1, H_sp* W_sp, C)  # (24,384,24)
    return img_perm
...
class Mlp(nn.Module):
    ...
    def forward(self, x):  # x(9216,48)
        ...
        x = rearrange(x, "b (h w) c -> b c h w", h=h, w=w)  # x(1,192,96,96)
        x1, x2 = x.chunk(2, dim=1)  # x1(1,96,96,96) x2(1,96,96,96)
        x = self.drop(self.act(x1))*x2 + self.drop(self.act(x2))*x1  # x(1,96,96,96)
        x = rearrange(x, "b c h w -> b (h w) c", h=h, w=w)  # x(9216,96)
...
class Attention_axial(nn.Module):
    ...
    def im2win(self, x, H, W):
        B, N, C = x.shape  # B=1 N=9216 C=24
        x = x.transpose(-2,-1).contiguous().view(B, C, H, W)  # x(24,96,96)
...
class CATB_axial(nn.Module):
    ...
    def forward(self, x, x_size):  # x(9216,48) x_size[96,96]
        """
        ...
        assert L == H * W, "flatten img_tokens has wrong size"
        img = self.norm1(x)
        qkv = self.qkv(img)  # qkv(9216,144)
        qkv = qkv.reshape(B, -1, 3, C).permute(2, 0, 1, 3) # 3, B, HW, C
        # v without partition
