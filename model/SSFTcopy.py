...
class CATB_axial(nn.Module):
    def __init__(self, dim, reso, num_heads,
        ...
        self.qkv = nn.Linear(dim//2, dim*3//2, bias=qkv_bias)
        ...
        self.get_v = dep_conv(in_channels=dim//2, kernel_size=3)  # Dep Conv
        ...
        self.f_attns = GlobalFrequencyBlock(in_channels=dim//2)
    ...
    def forward(self, x, x_size):  # x(1,9216,48) x_size[96,96]
        ...
        qkv = self.qkv(img[:,:,:C//2])  # qkv(1,9216,108)
        qkv = qkv.reshape(B, -1, 3, C//2).permute(2, 0, 1, 3)  # 3, B, HW, C qkv(3,1,9216,36)
        # v without partition
        v = qkv[2].transpose(-2, -1).contiguous().view(B, C//2, H, W)  # v(1,36,96,96)
        ...
        if self.shift_size > 0:
            ...
            # V-Rwin
            x1 = self.attns[0](qkv[:, :, :, :C//4], H, W).view(B, L, C//4).contiguous()  # x1(1,9216,18)
            # H-Rwin
            x2 = self.attns[1](qkv[:, :, :, C//4:C//2], H, W).view(B, L, C//4).contiguous()  # x1(1,9216,18)
            # Concat
        ...
        # 频率块
        attened_f = self.f_attns(img[:, :, C//2:], x_size)
        attened = torch.cat([attened_x, attened_x], dim=2)
...
class GlobalFrequencyBlock(nn.Module):
    ...
    def forward(self, x, x_size):  # x(1,9216,48)
        H, W = x_size
        x = rearrange(x, "b (h w) c -> b c h w", h=H, w=W).contiguous()  # x(16,24,96,96)
