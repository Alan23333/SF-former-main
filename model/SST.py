...
def window_partition(x, window_size):  # x(1,96,96,48)
    """
    Args:
        x: (B, H, W, C)
...
class NLSA(nn.Module):
    ...
    def forward(self, x, mask=None):  # x(144,64,48)
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        """
        B_, N, C = x.shape  # x(144,64,48)
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # qkv(3,144,6,64,8)
        q, k, v = qkv[0], qkv[1], qkv[2]  # q(144,6,64,8)

        q = q * self.scale  # q(144,6,64,8)
        attn = (q @ k.transpose(-2, -1))  # q(144,6,64,64)
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH  relative_position_bias(64,64,6)
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww  (6,64,64)
        attn = attn + relative_position_bias.unsqueeze(0)  # attn(144,6,64,64)

        if mask is not None:
            nW = mask.shape[0]  # nW=144
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0)  # attn(1,144,6,64,64)
            attn = attn.view(-1, self.num_heads, N, N)  # attn(144,6,64,64)
            attn = self.softmax(attn)  # attn(144,6,64,64)
        else:
            attn = self.softmax(attn)  # attn(144,6,64,64)

        attn = self.attn_drop(attn)  # attn(144,6,64,64)

        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)  # x(144,64,48)
        x = self.proj(x)  # x(144,64,48)
        x = self.proj_drop(x)  # x(144,64,48)
        return x
...
class SSMA(nn.Module):
    ...
    def calculate_mask(self, x_size):
        # calculate attention mask for SW-MSA  计算SW-MSA的注意力掩码
        H, W = x_size
    ...
    def forward(self, x):  # x(1,48,96,96)
        ...
        B,C ,H ,W = x.shape  # x(1,48,96,96)
        x = x.flatten(2).transpose(1, 2)  # x(1,9216,48)
        shortcut = x  # shortcut(1,9216,48)
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # x_windows(144,64,48)

        if self.input_resolution == [H,W ]:  # non-local speatial attention  非局部空间相似性
            attn_windows = self.attn(x_windows, mask=self.attn_mask)
        else:
            attn_windows = self.attn(x_windows, mask=self.calculate_mask([H,W ]).to(x.device))  # attn_windows(144,64,48)
        ...
        x = self.spectral_attn(x)  # global spectral attention  # x(1,48,96,96)  全局光谱注意力

        x = x.flatten(2).transpose(1, 2)  # x(1,9216,48)
        # FFN
        x = shortcut + self.drop_path(x)  # x(1,9216,48)
        x = x + self.drop_path(self.mlp(self.norm2(x)))  # x(1,9216,48)
...
class SST(nn.Module):
    ...
    def forward(self, inp_img):  # inp_img(1,10,96,96)
        ...
        for layer in self.layers:
            x = layer(x)  # x(1,48,96,96)

        x = self.output(x + f1)  # x(1,48,96,96)
