...
class SFTB(nn.Module):
    def __init__(self, dim, reso, num_heads,
        ...
        self.locality = dep_conv(in_channels=self.channels, kernel_size=3)  # Dep Conv
        ...
        self.f_branch = FrequencyAttention(in_channels=dim-self.channels)
    ...
    def forward(self, x, x_size):  # x(1,9216,48) x_size[96,96]
        ...
        qkv = self.qkv(img[:,:,:self.channels])  # qkv(1,9216,108)
        qkv = qkv.reshape(B, -1, 3, self.channels).permute(2, 0, 1, 3)  # 3, B, HW, C qkv(3,16,9216,24)
        # v without partition
        v = qkv[2].transpose(-2, -1).contiguous().view(B, self.channels, H, W)  # v(16,24,96,96)
        ...
        if self.shift_size > 0:
            ...
            # V-Rwin
            x1 = self.attns[0](qkv[:, :, :, :self.channels//2], H, W).view(B, L, self.channels//2).contiguous()  # x1(1,9216,12)
            # H-Rwin
            x2 = self.attns[1](qkv[:, :, :, self.channels//2: self.channels], H, W).view(B, L, self.channels//2).contiguous()  # x1(1,9216,12)
            # Concat
            attened_x = torch.cat([x1, x2], dim=2)  # x1(1,9216,24)

        # Locality Complementary Module
        lcm = self.locality(v)  # lcm(1,)  # lcm(1,24,96,96)
        lcm = lcm.permute(0, 2, 3, 1).contiguous().view(B, L, self.channels)  # lcm(1,9216,48)
        attened_x = attened_x + lcm  # attened_x(1,9216,24)

        # 频率块
        attened_f = self.f_branch(img[:, :, self.channels:], x_size)
