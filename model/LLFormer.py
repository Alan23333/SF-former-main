...
class LLFormer(nn.Module):
    ...
    def forward(self, inp_img):  # inp_img(1,10,96,96)

        inp_enc_encoder1 = self.patch_embed(inp_img)  # inp_enc_encoder1(1,16,96,96)
        out_enc_encoder1 = self.encoder_1(inp_enc_encoder1)  # out_enc_encoder1(1,16,96,96)
        out_enc_encoder2 = self.encoder_2(out_enc_encoder1)  # out_enc_encoder2(1,16,96,96)
        out_enc_encoder3 = self.encoder_3(out_enc_encoder2)  # out_enc_encoder3(1,16,96,96)


        inp_fusion_123 = torch.cat([out_enc_encoder1.unsqueeze(1),out_enc_encoder2.unsqueeze(1),out_enc_encoder3.unsqueeze(1)],dim=1)  # inp_fusion_123(1,3,16,96,96)

        out_fusion_123 = self.layer_fussion(inp_fusion_123)  # out_fusion_123(1,48,96,96)
        out_fusion_123 = self.conv_fuss(out_fusion_123)  # out_fusion_123(1,16,96,96)


        inp_enc_level1_0 = self.down_1(out_fusion_123)  # inp_enc_level1_0(1,32,48,48)
        out_enc_level1_0 = self.decoder_level1_0(inp_enc_level1_0)  # out_enc_level1_0(1,32,48,48)



        inp_enc_level2_0 = self.down_2(out_enc_level1_0)  # inp_enc_level2_0(1,64,24,24)
        out_enc_level2_0 = self.decoder_level2_0(inp_enc_level2_0)  # out_enc_level2_0(1,64,24,24)



        inp_enc_level3_0 = self.down_3(out_enc_level2_0)  # inp_enc_level3_0(1,128,12,12)
        out_enc_level3_0 = self.decoder_level3_0(inp_enc_level3_0)  # out_enc_level3_0(1,128,12,12)



        inp_enc_level4_0 =   self.down_4(out_enc_level3_0)  # inp_enc_level4_0(1,256,6,6)
        out_enc_level4_0 = self.decoder_level4(inp_enc_level4_0)  # out_enc_level4_0(1,256,6,6)


        out_enc_level4_0 = self.up4_3(out_enc_level4_0)  # out_enc_level4_0(1,128,12,12)

        inp_enc_level3_1 = self.coefficient_4_3[0, :][None, :, None, None] * out_enc_level3_0 + self.coefficient_4_3[1, :][None, :, None, None] * out_enc_level4_0  # inp_enc_level3_1(1,128,12,12)
        inp_enc_level3_1 = self.skip_4_3(inp_enc_level3_1)  # inp_enc_level3_1(1,128,12,12)  ### conv 1x1


        out_enc_level3_1 = self.decoder_level3_1(inp_enc_level3_1)  # inp_enc_level3_1(1,128,12,12)


        out_enc_level3_1 = self.up3_2(out_enc_level3_1)  # out_enc_level3_1(1,64,24,24)
        inp_enc_level2_1 = self.coefficient_3_2[0, :][None, :, None, None] * out_enc_level2_0 + self.coefficient_3_2[1, :][None, :, None, None] * out_enc_level3_1  # inp_enc_level2_1(1,64,24,24)
        inp_enc_level2_1 = self.skip_3_2(inp_enc_level2_1)  # inp_enc_level2_1(1,64,24,24)  ### conv 1x1


        out_enc_level2_1 = self.decoder_level2_1(inp_enc_level2_1)  # out_enc_level2_1(1,64,24,24)

        out_enc_level2_1 = self.up2_1(out_enc_level2_1)  # out_enc_level2_1(1,32,48,48)

        inp_enc_level1_1 = self.coefficient_2_1[0, :][None, :, None, None] * out_enc_level1_0 + self.coefficient_2_1[1, :][None, :, None, None] *  out_enc_level2_1  # inp_enc_level1_1(1,32,48,48)

        inp_enc_level1_1 = self.skip_1_0(inp_enc_level1_1)  # inp_enc_level1_1(1,32,48,48)  ### conv 1x1

        out_enc_level1_1 = self.decoder_level1_1(inp_enc_level1_1)  # out_enc_level1_1(1,32,48,48)

        out_enc_level1_1 = self.up2_0(out_enc_level1_1)  # out_enc_level1_1(1,16,96,96)


        out_fusion_123 = self.latent(out_fusion_123)  # out_fusion_123(1,16,96,96)  这里就是最上面中间多出来的块


        out = self.coefficient_1_0[0, :][None, :, None, None] * out_fusion_123  + self.coefficient_1_0[1, :][None, :, None, None] *  out_enc_level1_1  # out(1,16,96,96)

        out_1 = self.refinement_1(out)  # out_1(1,16,96,96)

        out_2 = self.refinement_2(out_1)  # out_2(1,16,96,96)
        out_3 = self.refinement_3(out_2)  # out_3(1,16,96,96)
        inp_fusion = torch.cat([out_1.unsqueeze(1),out_2.unsqueeze(1),out_3.unsqueeze(1)],dim=1)  # inp_fusion(1,3,16,96,96)
        out_fusion_123 = self.layer_fussion_2(inp_fusion)  # out_fusion_123(1,48,96,96)
        out = self.conv_fuss_2(out_fusion_123)  # out(1,16,96,96)

        if self.skip:
            out = self.output(out)+ inp_img
        else:
            out = self.output(out)  # out(1,10,96,96)
...
class NextAttentionImplZ(nn.Module):
    ...
    def forward(self, x):  # x(1,16,96,96)
        # x: [n, c, h, w]
        n, c, h, w = x.size()  # 1 16 96 96
        n_heads, dim_head = self.num_heads, c // self.num_heads  # n_heads=1 dim_head=16
        reshape = lambda x: einops.rearrange(x, "n (nh dh) h w -> (n nh h) w dh", nh=n_heads, dh=dim_head)

        qkv = self.q3(self.q2(self.q1(x)))  # qkv(1,48,96,96)
        q, k, v = map(reshape, qkv.chunk(3, dim=1))  # q(96,96,16)
        q = F.normalize(q, dim=-1)  # q(96,96,16)
        k = F.normalize(k, dim=-1)  # k(96,96,16)

        # fac = dim_head ** -0.5
        res = k.transpose(-2, -1)  # res(96,16,96)
        res = torch.matmul(q, res) * self.fac  # res(96,96,96)
        res = torch.softmax(res, dim=-1)  # res(96,96,96)

        res = torch.matmul(res, v)  # res(96,96,16)
        res = einops.rearrange(res, "(n nh h) w dh -> n (nh dh) h w", nh=n_heads, dh=dim_head, n=n, h=h)  # qkv(1,16,96,96)
        res = self.fin(res)  # qkv(1,16,96,96)
...
class NextAttentionZ(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        assert len(x.size()) == 4  # x(16,96,96)

        x = self.row_att(x)  # x(1,16,96,96)
        x = x.transpose(-2, -1)  # x(1,16,96,96)
        x = self.col_att(x)  # x(1,16,96,96)
        x = x.transpose(-2, -1)  # x(1,16,96,96)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):  # x(16,96,96)
        x = self.project_in(x)  # x(84,96,96)
        x1, x2 = self.dwconv(x).chunk(2, dim=1)  # x1(42,96,96) x2(42,96,96)
        x = F.gelu(x2)*x1 + F.gelu(x1)*x2  # x(42,96,96)
        x = self.project_out(x)  # x(16,96,96)
        return x
