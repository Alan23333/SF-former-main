...
def img2windows(img, H_sp, W_sp):
    ...
    img: B C H W
    """
    B, C, H, W = img.shape  # img(144,24,8,8)
    img_reshape = img.view(B, C, H // H_sp, H_sp, W // W_sp, W_sp)  # img_reshape(144,24,1,8,8,1)
    img_perm = img_reshape.permute(0, 2, 4, 3, 5, 1).contiguous().reshape(-1, H_sp* W_sp, C)  # img_perm(1152,8,24)
    return img_perm
...
class LePEAttention(nn.Module):
    ...
    def im2cswin(self, x):
        B, N, C = x.shape  # x(144,64,24)
        H = W = int(np.sqrt(N))
        x = x.transpose(-2 ,-1).contiguous().view(B, C, H, W)  # x(144,24,8,8)
        x = img2windows(x, self.H_sp, self.W_sp)  # x(1152,8,24)
        x = x.reshape(-1, self.H_sp* self.W_sp, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()  # x(1152,3,8,8)
        return x

    def get_lepe(self, x, func):  # x(144,64,24)
        B, N, C = x.shape
        H = W = int(np.sqrt(N))  # H=W=8
        x = x.transpose(-2 ,-1).contiguous().view(B, C, H, W)  # x(144,24,8,8)

        H_sp, W_sp = self.H_sp, self.W_sp  # H_sp=8 W_sp=1
        x = x.view(B, C, H // H_sp, H_sp, W // W_sp, W_sp)  # x(144,24,1,8,8,1)
        x = x.permute(0, 2, 4, 1, 3, 5).contiguous().reshape(-1, C, H_sp, W_sp) ### B', C, H', W'  x(1152,24,8,1)

        lepe = func(x)  # lepe(1152,24,8,1)  ### B', C, H', W'
        lepe = lepe.reshape(-1, self.num_heads, C // self.num_heads, H_sp * W_sp).permute(0, 1, 3, 2).contiguous()  # lepe(1152,3,8,8)

        x = x.reshape(-1, self.num_heads, C // self.num_heads, self.H_sp* self.W_sp).permute(0, 1, 3, 2).contiguous()  # x(1152,3,8,8)
        return x, lepe

    def forward(self, qkv ,mask=None): # qkv(3,144,64,24)
        """
        x: B L C
        """
        q ,k ,v = qkv[0], qkv[1], qkv[2]  # q(144,64,24)

        ### Img2Window
        H = W = self.resolution  # H=W=8
        B, L, C = q.shape  # q(144,64,24)
        # assert L == H * W, "flatten img_tokens has wrong size"

        q = self.im2cswin(q)  # q(1152,3,8,8)


        k = self.im2cswin(k)  # k(1152,3,8,8)
        v, lepe = self.get_lepe(v, self.get_v)  # v(1152,3,8,8) lepe(1152,3,8,8)

        q = q * self.scale  # q(1152,3,8,8)
        # print(q.shape,k.shape)
        attn = (q @ k.transpose(-2, -1))  # attn(1152,3,8,8)  # B head N C @ B head C N --> B head N N

        attn = nn.functional.softmax(attn, dim=-1, dtype=attn.dtype)  # attn(1152,3,8,8)
        attn = self.attn_drop(attn)  # attn(1152,3,8,8)

        x = (attn @ v) + lepe  # x(1152,3,8,8)
        x = x.transpose(1, 2).reshape(-1, self.H_sp* self.W_sp, C)  # B head N N @ B head N C  x(1152,8,24)

        ### Window2Img
        x = windows2img(x, self.H_sp, self.W_sp, H, W).view(B, -1, C)  # B H' W' C  x(144,64,24)
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):  # (144,64,48)
        b, n, c = x.shape  # x(144,64,48)  144个块，每个块8*8
        ...
        y1 = f_dic_c @ mbg.transpose(1, 2)  # y1(144,1,3)
        y2 = self.upnet(y1)  # (144,1,48)
        out = x * y2  # (144,64,48)
        return out
...
class CAB(nn.Module):
    ...
    def forward(self, x):
        return self.cab(x)  # x(144,64,48) 144个块，每个块8*8
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        qkv = self.qkv(x).reshape(B, -1, 3, C).permute(2, 0, 1, 3)  # qkv(3,144,64,48)

        x1 = self.attns[0](qkv[:, :, :, :C // 2], mask)  # x1(144,64,24)  # 分为两种注意力
        x2 = self.attns[1](qkv[:, :, :, C // 2:], mask)  # x2(144,64,24)

        attened_x = torch.cat([x1, x2], dim=2)  # attened_x(144,64,48)

        attened_x = rearrange(attened_x, 'b n (g d) -> b n ( d g)', g=4)  # attened_x(144,64,48) 光谱分流模块

        x3 = self.c_attns(x)  # 光谱增强模块  # x(144,64,48) 144个块，每个块8*8
...
class SSMTDA(nn.Module):
    ...
    def forward(self, x):  # x(1,48,96,96)
        ...
        B, C, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)  # x(1,9216,48)
        shortcut = x  # shortcut(1,9216,48)  留作残差
        x = self.norm1(x)  # x(1,9216,48)
        x = x.view(B, H, W, C)  # x(1,96,96,48)

        # cyclic shift 循环移位操作
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        else:
            shifted_x = x  # shifted_x(1,96,96,48)

        # partition windows  窗口分区
        x_windows = window_partition(shifted_x, self.window_size)  # x_windows(144,8,8,48)
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # x_windows(144,64,48)  144个块，每个块8*8  通道为48

        attn_windows = self.attns(x_windows)  # attn_windows(144,64,48)
        ...
        if self.shift_size > 0:
            ...
            x = shifted_x  # x(1,96,96,48)

        x = x.view(B, H * W, C)  # x(1,9216,48)


        x = shortcut + self.drop_path(x)  # x(1,96,96,48)
        # FFN
        x = x + self.drop_path(self.mlp(self.norm2(x)))  # x(1,96,96,48)

        x = x.transpose(1, 2).view(B, C, H, W)  # x(1,48,96,96)
...
class SMSBlock(nn.Module):
    ...
    def forward(self, x):  # x(1,48,96,96)
        out = self.smsblock(x)  # x(1,48,96,96)
        out = self.conv(out) + x  # x(1,48,96,96)
        return out
...
class SERT(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 6, 6, 6, 6, 6],  # 所以有6个Transformer块，每个块中循环6次
    ...
    def forward(self, inp_img):  # inp_img(1,10,96,96)
        _, _, h_inp, w_inp = inp_img.shape  # h_inp=96 w_inp=96
        hb, wb = 16, 16
        pad_h = (hb - h_inp % hb) % hb  # pad_h=0
        pad_w = (wb - w_inp % wb) % wb  # pad_w=0
        inp_img = F.pad(inp_img, (0, pad_h, 0, pad_w), 'reflect')  # inp_img(1,10,96,96)  相当于正好不用padding
        f1 = self.conv_first(inp_img)  # f1(1,48,96,96)
        x = f1  # x(1,48,96,96)  用来做残差
        for layer in self.layers:  # Transformer块
            x = layer(x)
